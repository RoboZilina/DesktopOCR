...
| **Offline Capability** | Internet required for initial model download; Cloudflare needed for COOP/COEP; browser must remain open | Partially offline — OCR and local TTS (OpenJTalk) work without internet; translation (DeepL/Google) and AI validation require cloud APIs |
...
| **Translation & TTS** | Browser Web Speech API (limited system voices; no Japanese-specific engines); no translation | Multi-backend TTS: Edge TTS (cloud), OpenJTalk (local), COEIROINK (local, requires server); translation via DeepL (free web endpoint, needs internet) or Google Translate (needs internet); LibreTranslate (local, not yet bundled) |
...
- You want **offline OCR and TTS** — models bundled locally; translation (DeepL/Google) and AI validation still need internet
- You need **Anki integration** to create flashcards directly from captured text
- You want **AI-assisted validation** via DeepSeek or OpenAI, and **translation** via DeepL or Google Translate
