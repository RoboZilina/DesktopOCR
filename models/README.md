...
|---|---|---|
| `det.onnx` | PP-OCRv5 server detection model | ~86 MB |
| `rec.onnx` | PP-OCRv5 server recognition model | ~82 MB |
...
Download the files from that repository and place them directly into `models/paddle/` with the exact filenames listed above (or rename them to `det.onnx` and `rec.onnx`). The application will not start if any of these files are missing.
