...
- **Impact**: Auto-mode falls back to MyMemory → Argos if Google breaks.
...
- **Issue**: `mymemory_backend.py` and `google_backend.py` have no unit tests. `argos_backend.py` is covered by `tests/test_argos_backend.py` (assertions + offline). `tests/test_translation.py` exercises the full pipeline.
- **Risk**: Regressions in cloud API backends (session handling, response parsing) go undetected without internet.

### `argostranslate` dependency untested on target Windows Python 3.11
- **Requirement**: `argostranslate==1.9.6` added to `requirements.txt`
- **Issue**: `argostranslate` depends on `ctranslate2` which ships Windows wheels, but the full model download + install flow has never been run end-to-end in this project. First-use model download may fail behind proxies or corporate firewalls.
- **Risk**: Argos backend silently returns `""` (all errors caught), user gets no translation with no visible error.

### No crash reporting in release builds
- **Issue**: The PyInstaller build produces a console-less GUI EXE (`console=False`). Any unhandled exception is silently swallowed — the app window disappears with no diagnostic trace.
...
| LibreTranslateBackend | Dead | File kept on disk but module is not imported anywhere; replaced by ArgosTranslate |
| DeepLBackend | Dead | File kept on disk but not imported anywhere; replaced by MyMemory |
